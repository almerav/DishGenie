...
# Setup the PaLM API client using the secret API key
palm.configure(api_key=st.secrets["API_key"]["api_key"])

# Function to generate response from PaLM API
...
def main():
    st.title("DishGenie")
    st.subheader("DishGenie allows users to find dishes based on their preferred cuisine and available ingredients using Google Generative AI")
